...
var Twitter = function (oauth, options) {
  ...
  this.oauth = oauth
  this.name = options.name ? options.name : 'App name'

  // we assume minimal 2 app is set. so we don't do backoff, just init connection with another app
  this.doBackoff = options.doBackoff ? options.doBackoff : false;
...
Twitter.prototype.untrackMultiple = function (keywords, reconnect) {
  reconnect = typeof reconnect === 'undefined' || reconnect
  for (index in keywords) {
    this.removeFilter(FILTER_TYPE_TRACKING, keywords[index], false)
  }
...
Twitter.prototype.connect = function () {
  ...
  if (this.name) {
    console.log('TwitterStream for app: ' + this.name + '. Tracking ' + Object.keys(this._filters[FILTER_TYPE_TRACKING]).join(','));
  }
...
Twitter.prototype.abortFromOutside = function () {
  var self = this
  self.abort();
}
